$(document).ready(function(){
    ...
    /* skillz */
    var skillzStarted = false;

    var startSkillz = function (){
        if (skillzStarted){ return false; }

        var ele = '.skillbarSection';
        var delay = 300;

        var scroll = $(window).scrollTop();
        var pageBot = scroll + $(window).height();
        var eleTop = $(ele).offset().top;
        var eleBot = eleTop + $(ele).height();

        var isInView = ((eleBot <= pageBot) && (eleTop >= scroll));
